  ...
  const handleDeleteUser = async (userId) => {
    if (window.confirm('Вы уверены, что хотите удалить этого пользователя?')) {
      try {
        ...
        if (err.response?.data?.error === 'cannot delete the last admin user') {
          setError('Невозможно удалить последнего администратора');
        } else {
              ...
              <FormControlLabel
                control={
                  <Switch
                    ...
                    disabled={selectedUser.is_admin && users.filter(u => u.is_admin).length <= 1}
                  />
                }
                label="Администратор"
              />
